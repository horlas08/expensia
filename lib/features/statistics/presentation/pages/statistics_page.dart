...
final statisticsDataProvider = FutureProvider.autoDispose.family<_StatsData, int>((ref, period) async {
  ...
  if (period == 0) {
    ...
    starts = List.generate(7, (i) => DateTime(monday.year, monday.month, monday.day + i));
  } else if (period == 1) {
    // Last 6 months
  ...
  ''');
  final installmentTotal = (instRes.first['total'] as num?)?.toDouble() ?? 0;
                  ...
                  statsAsync.when(
                    ...
                    data: (stats) {
                      final maxY = [...stats.income, ...stats.expense].isEmpty
                          ? 100.0
                          : ([...stats.income, ...stats.expense].reduce(math.max) * 1.25).ceilToDouble();
                      final totalSpent = stats.categories.fold(0.0, (s, c) => s + c.amount);
                                ...
                                child: AnimatedBuilder(
                                  ...
                                  builder: (_, __) => BarChart(
                                    BarChartData(
                                        ...
                                        bottomTitles: AxisTitles(
                                          sideTitles: SideTitles(
                                            ...
                                            getTitlesWidget: (val, meta) {
                                              final i = val.toInt();
                                              if (i >= stats.labels.length) return const SizedBox();
                                              return Padding(
                                                padding: const EdgeInsets.only(top: 6),
                                                child: Text(stats.labels[i], style: TextStyle(fontSize: 10, color: cs.onSurfaceVariant)),
                                              );
                                            },
                                          ),
                                        ),
                                      ...
                                      barGroups: List.generate(stats.income.length, (i) {
                                        ...
                                        return BarChartGroupData(
                                          ...
                                          barRods: [
                                            BarChartRodData(
                                              toY: stats.income[i] * _progressAnim.value,
                                            ...
                                            BarChartRodData(
                                              toY: stats.expense[i] * _progressAnim.value,
                                              gradient: const LinearGradient(
                                  ...
                                  : Column(
                                      children: [
                                        SizedBox(
                                          ...
                                          child: Stack(
                                            ...
                                            children: [
                                              PieChart(
                                                PieChartData(
                                                  sections: List.generate(stats.categories.length, (i) {
                                                    final cat = stats.categories[i];
                                        ...
                                        ...stats.categories.map((cat) {
                                          final pct = totalSpent > 0 ? cat.amount / totalSpent : 0.0;
                                          return Padding(
